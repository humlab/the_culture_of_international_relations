...
def display_gui(wti_index, display_function):
    ...
    treaty_filter_widget = widgets_config.treaty_filter_widget(layout=lw())
    ...
    n_top_widget = widgets_config.slider('Top/grp #:', 2, 100, 25, step=10)
    n_min_count_widget = widgets_config.slider('Min count:', 1, 10, 5, step=1, tooltip='Filter out words with count less than specified value')
    ...
    boxes = widgets.HBox(
        [
            ...
            widgets.VBox([ treaty_filter_widget]),
            widgets.VBox([ output_format_widget, progress_widget ])
        ]
    )
