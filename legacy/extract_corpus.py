...
def display_document_key_terms_gui(corpus, wti_index):
    ...
    def get_document_key_terms(corpus, method="TextRank", document_id=None, normalize="lemma", n_keyterms=10):
        ...
        keyterms = [
            get_keyterms(method, corpus_utility.get_treaty_doct(corpus, treaty_id), normalize, n_keyterms)
...
def display_cleaned_up_text(
    container, gui, display_type, treaty_id, gpe_substitutions, word_count_scores, word_document_count_scores, **opts
):
    ...
    try:
        ...
        if display_type.startswith("source_text"):
            ...
            source_files = {
                "source_text_raw": {
                    "filename": container.source_path,
                    "description": "Raw text from PDF: Automatic text extraction using pdfminer Python package. ",
                },
                "source_text_edited": {
                    "filename": container.source_path,
                    "description": "Manually edited text: List of references, index, notes and page headers etc. removed.",
                },
                "source_text_preprocessed": {
                    "filename": container.prepped_source_path,
                    "description": "Preprocessed text: Normalized whitespaces. Unicode fixes. Urls, emails and phonenumbers removed. Accents removed.",
                },
            }
...
def display_cleanup_text_gui(container, wti_index):
    ...
    gui = types.SimpleNamespace(
        position=1,
        document_id=widgets.Dropdown(
            description="Treaty", options=document_options, value=document_options[1][1], layout=lw("400px")
        ),
        ...
        max_doc_freq=widgets.IntSlider(
            description="Max doc. %", min=0, max=100, value=100, step=1, layout=widgets.Layout(width="400px")
        ),
        mask_gpe=widgets.ToggleButton(
        ...
        [
            ...
            widgets.HBox(
                [
                    ...
                    widgets.VBox([gui.filter_stops, gui.mask_gpe, gui.filter_punct, gui.named_entities]),
                ]
            ),
            widgets.HBox([gui.output_text, gui.output_statistics]),
            uix.children[-1],
        ]
    )
...
def compute_list_of_most_frequent_words(
    ...
    if display_score is True:
        df_freqs["term"] = (
            df_freqs.term
            + "*"
            + (df_freqs.score.apply("{:,.3f}".format) if weighting == "freq" else df_freqs.score.astype(str))
        )

    df_freqs["position"] = (
        df_freqs.sort_values(by=[group_by_column, "score"], ascending=False).groupby([group_by_column]).cumcount() + 1
    )
...
def display_list_of_most_frequent_words(gui, df):
    if gui.output_type.value == "table":
        ...
        df_unstacked_freqs = (
            df[[group_by_column, "position", "term"]].set_index([group_by_column, "position"]).unstack()
        )
...
def word_frequency_gui(wti_index, corpus, compute_callback, display_callback):
    ...
    frequent_words = [
        x[0] for x in corpus_utility.get_most_frequent_words(corpus, 100, include_pos=default_include_pos)
    ]
