...
def complete_missing_data_points(data, period_group, category_column="Party", value_column="Count"):
    ...
    periods = (
        ...
        else ["{} to {}".format(x[0], x[1]) for x in period_group["periods"]]
    )

    period_categories = pd.DataFrame(list(product(periods, categories)), columns=["Period", category_column]).set_index(
        ["Period", category_column]
    )
...
class QuantityByParty:
    ...
    def get_top_parties(stacked_treaties, period_group, party_name, n_top=3):
        ...
        top_list = (
            xd.groupby([period_column])
            .apply(lambda x: x.nlargest(n_top, "TopCount"))
    ...
    def get_treaties_statistics(
        ...
        *,
        ...
        extra_category: str = "",
        n_top: int = 0,
        year_limit: None | Sequence[int] = None,
        treaty_sources: None | list[str] = None,
    ) -> pd.DataFrame:

        period_column: str = period_group["column"]

        treaty_subset: pd.DataFrame = wti_index.get_treaties_within_division(
            wti_index.treaties,
            period_group,
        ...
        df = wti_index.stacked_treaties.loc[
            treaty_subset.index
        ]  #  wti_index.stacked_treaties[wti_index.stacked_treaties.index.isin(treaty_subset.index)]
        ...
        if "ALL" in parties:
            ...
            ]  # de avtal som vars länder valts
        else:
            df_top: pd.DataFrame = QuantityByParty.get_top_parties(
                df, period_group=period_group, party_name=party_name, n_top=n_top
        ...
        if extra_category == "other_category":
            ...
            extra_party: dict[str, Any] | None = wti_index.get_party("ALL OTHER")

        elif extra_category == "all_category":
            df_extra = df[df.index.isin(treaty_subset.index) & (~df.reversed)]
        ...
        data: pd.DataFrame = (
            treaties_of_parties.groupby([period_column, party_name])
            .size()
            .reset_index()
...
class QuantityByTopic:
    ...
    def get_treaty_topic_quantity_stat(
        ...
        treaties: pd.DataFrame = wti_index.treaties.copy()
        parties = party_group["parties"]

        categorized_treaties: pd.DataFrame = wti_index.get_categorized_treaties(
            treaties,
        ...
        if target_quantity == "topic":
            ...
            data: pd.DataFrame = (
                parties_treaties.groupby([period_group["column"], target_column[target_quantity]])
                .size()
                .reset_index()
                .rename(
                    columns={period_group["column"]: "Period", target_column[target_quantity]: "Category", 0: "Count"}
                )
            )
            ...
            stacked_treaties: pd.DataFrame = pd.merge(
                wti_index.stacked_treaties[["party", "party_other"]],
                categorized_treaties,
                left_on="treaty_id",
                right_index=True,
                how="inner",
            )

            party_continents: pd.DataFrame = wti_index.parties[["continent_code", "group_name"]]
            stacked_treaties = pd.merge(
                stacked_treaties, party_continents, left_on=["party_other"], right_index=True, how="inner"
            ...
            data = (
                stacked_treaties.groupby([period_group["column"], target_column[target_quantity]])
                .size()
                .reset_index()
                .rename(
                    columns={period_group["column"]: "Period", target_column[target_quantity]: "Category", 0: "Count"}
