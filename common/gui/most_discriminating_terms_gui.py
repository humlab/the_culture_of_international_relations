...
def compute_most_discriminating_terms(
    ...
    def get_term_list(corpus, region=None, period=None, closed_region=False):
        ...
        if period is not None:
            docs = (
                x for x in docs if (period[0] <= x._.meta["signed_year"]) and (x._.meta["signed_year"] <= period[1])
            )
...
def display_most_discriminating_terms(df):
    ...
    print(
        "No additional filters are applied apart from selections made on Parties and Year in this CELL i.e. WTI index selection has no effect in this notebook!"
    )
...
def display_gui(wti_index, corpus):
    ...
    gui = types.SimpleNamespace(
        ...
        group1=widgets.SelectMultiple(
            description="Group 1", options=parties_options, value=[], rows=7, layout=lw("250px")
        ),
        group2=widgets.SelectMultiple(
            description="Group 2", options=parties_options, value=[], rows=7, layout=lw("250px")
        ),
        ...
        include_pos=widgets.SelectMultiple(
            description="POS", options=include_pos_tags, value=include_pos_tags, rows=7, layout=lw("150px")
        ),
        period1=widgets.IntRangeSlider(
            description="Period", min=period_default[0], max=period_default[1], value=period_default, layout=lw("250px")
        ),
        period2=widgets.IntRangeSlider(
            description="Period", min=period_default[0], max=period_default[1], value=period_default, layout=lw("250px")
        ),
        closed_region=widgets.ToggleButton(
            description="Closed regions", icon="check", value=True, disabled=False, layout=lw("140px")
        ),
    ...
    boxes = widgets.VBox(
        [
            widgets.HBox(
                [
                    widgets.VBox([gui.group1, gui.group1_preset, gui.period1]),
                    widgets.VBox([gui.group2, gui.group2_preset, gui.period2]),
                    widgets.VBox(
                        [
                            gui.include_pos,
                            gui.closed_region,
    ...
    def compute_callback_handler(*_args):
        ...
        with gui.output:
            try:
                ...
                df: None | pd.DataFrame = compute_callback(
                    wti_index=wti_index,
                    corpus=corpus,
