...
def compute_co_occurrance(treaty_tokens) -> pd.DataFrame:
    ...
    co_occurrance["token"] = co_occurrance.apply(
        lambda row: " - ".join([row["token_x"].upper(), row["token_y"].upper()]), axis=1
    )
    co_occurrance["lemma"] = co_occurrance.apply(
        lambda row: " - ".join([row["lemma_x"].upper(), row["lemma_y"].upper()]), axis=1
    )

    co_occurrance: pd.DataFrame = co_occurrance.assign(is_stopword=False, sequence_id=0)[
        ["sequence_id", "token", "lemma", "is_stopword"]
    ]
...
def display_headnote_toplist(
    ...
    *,
    ...
    recode_is_cultural: bool | None = False,
    treaty_filter: dict[str, str] | None = None,
    parties: list[str] | None = None,
    extra_groupbys=None,
    use_lemma: bool = False,
    compute_co_occurance: bool = False,
    ...
    try:
        if print_args:
            args: dict[str, Any] = utility.filter_dict(locals(), {"progress", "print_args"}, filter_out=True)
            args["wti_index"] = None
            pp(args)

        corpus: headnote_corpus.HeadnoteTokenCorpus = headnote_corpus.HeadnoteTokenCorpus(treaties=wti_index.treaties)

        period_group: dict[str, str] = config.DEFAULT_PERIOD_GROUPS[period_group_index]
        topic_group: dict[str, str] = config.TOPIC_GROUP_MAPS[topic_group_name]

        progress()

        # Get subset of treaties filtered by topic, period, and is-cultural filter
        treaties: pd.DataFrame = wti_index.get_categorized_treaties(
        ...
        treaty_tokens: pd.DataFrame
        if compute_co_occurance:
            treaty_tokens = corpus.get_tokens_for(treaties.index)
            treaty_tokens = (
                treaty_tokens.loc[treaty_tokens[token_or_lemma].str.len() >= min_word_size]
                .reset_index()
            ...
            if remove_stopwords is True:
                treaty_tokens = treaty_tokens.loc[~treaty_tokens.is_stopword]
        ...
        result: pd.DataFrame = (
            result.groupby(groupbys[-1]).apply(lambda x: x.nlargest(n_top, "Count")).reset_index(level=0, drop=True)
        )
        ...
        if output_format in ("table", "tabular"):
            result.columns = [remove_snake_case(x) for x in result.columns]
            if output_format == "table":
                display(result)
            else:
                grid: DataGrid = DataGrid(result, selection_mode="row", layout={"height": "400px"})
                display(grid)

        elif output_format == "unstack":
            result = result.set_index(groupbys).unstack(level=0).fillna(0).astype("int32")  # type: ignore
            result.columns = [x[1] for x in result.columns]
            display(result)
        elif output_format.startswith("plot"):
            parts: list[str] = output_format.split("_")
            kind: str = parts[-1]
            stacked: bool = "stacked" in parts
            result = result.set_index(list(reversed(groupbys))).unstack(level=0).fillna(0).astype("int32")  # type: ignore
            result.columns = [x[1] for x in result.columns]
            result.plot(kind=kind, stacked=stacked, figsize=(16, 8))  # type: ignore
...
def display_gui(wti_index, print_args=False):
    ...
    recode_7corr_widget = widgets_config.toggle(
        "Recode 7CULT", True, tooltip="Recode all treaties with cultural=yes as 7CORR", layout=lw(width="110px")
    )
    use_lemma_widget = widgets_config.toggle("Use LEMMA", False, tooltip="Use WordNet lemma", layout=lw(width="110px"))
    remove_stopwords_widget = widgets_config.toggle(
        "Remove STOP", True, tooltip="Do not include stopwords", layout=lw(width="110px")
    )
    compute_co_occurance_widget = widgets_config.toggle(
        "Cooccurrence", True, tooltip="Compute Cooccurrence", layout=lw(width="110px")
    )
    ...
    n_min_count_widget = widgets_config.slider(
        "Min count",
        1,
        10,
        5,
    ...
    boxes = widgets.HBox(
        [
            ...
            widgets.VBox(
                [topic_group_name_widget, extra_groupbys_widget, n_top_widget, min_word_size_widget, n_min_count_widget]
            ),
