...
class TreatyCompressedFileReader(CompressedFileReader):
    ...
    def __init__(self, path: str, language: str, treaty_ids: Sequence[str]) -> None:
    ...
    def __next__(self) -> tuple[str | int, str | int, str, str]:
        ...
        treaty_id: str | int = m.groups(1)[0]
        language: str | int = m.groups(1)[1]

        return treaty_id, language, filename, content


def get_document_stream(corpus_path: str, lang, treaties: pd.DataFrame):
    ...
    for treaty_id, language, filename, text in documents:
        ...
        metadata: pd.Series | pd.DataFrame = treaties.loc[treaty_id]
...
class TreatyCorpus(TextCorpus):
    ...
    def getstream(self):
        ...
        for treaty_id, language, filename, content in self.stream:
            yield content
            document_infos.append({"document_name": filename, "treaty_id": treaty_id, "language": language})
...
class MmCorpusStatisticsService:
    ...
    def __init__(self, corpus, dictionary, language) -> None:
        self.corpus = corpus
    ...
    def compute_word_stats(self) -> pd.DataFrame:
        df: pd.DataFrame = self.compute_document_stats()[["size", "stopwords"]]
        ...
        df_agg: pd.DataFrame = df_agg.set_index("index")
...
class TreatyCorpusSaveLoad:
    ...
    def __init__(self, source_folder, lang) -> None:

        self.mm_filename: str = os.path.join(source_folder, f"corpus_{lang}.mm")
        self.dict_filename: str = os.path.join(source_folder, f"corpus_{lang}.dict.gz")
        self.document_index: str = os.path.join(source_folder, f"corpus_{lang}_documents.csv")

    def store_as_mm_corpus(self, treaty_corpus) -> None:
