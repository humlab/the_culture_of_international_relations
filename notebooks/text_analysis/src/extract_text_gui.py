...
def tokenize_docs(docs, **opts): 
    try:
        ...
        if opts['min_freq'] > 1:
            stop_words: set[str] = utility.extract_counter_items_within_threshold(word_counts[normalize], 1, opts['min_freq'])
...
def store_tokenized_corpus(tokenized_docs, corpus_source_filepath, **opts) -> tuple[str, pd.DataFrame]: 
...
def display_generate_tokenized_corpus_gui(corpus, corpus_source_filepath, subst_filename=None):
    ...
    pos_tags: dict[str, str] = config.get_tag_set().groupby(['POS'])['DESCRIPTION'].apply(list).apply(lambda x: ', '.join(x[:1])).to_dict()
    pos_options: list[tuple[str, str] | tuple[str, None]] = [('(All)', None)] + sorted([(k + ' (' + v + ')', k) for k,v in pos_tags.items() ])
    ...
    word_counts: dict[str, dict[int, set[str]]] = { k: textacy_utility.generate_word_count_score(corpus, k, gui.min_freq.max) for k in [ 'lemma', 'lower', 'orth' ] }
    
    logger.info('...word document count...')
    word_document_counts: dict[str, dict[int, set[str]]] = { k: textacy_utility.generate_word_document_count_score(corpus, k, gui.max_doc_freq.min) for k in [ 'lemma', 'lower', 'orth' ] }
